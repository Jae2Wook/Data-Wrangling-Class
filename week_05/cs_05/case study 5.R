...
# male
library(downloader)
zip <- download("https://byuistats.github.io/M335/data/heights/Heights_south-east.zip", dest="dataset.zip", mode="wb") 
unzipped <- unzip("dataset.zip")
zip1 <- read.dbf(unzipped)
View(zip1)
...
# use haven, use import SPSS
# codebook: [https://www.ssc.wisc.edu/nsfh/wave3/NSFH3%20Apr%202005%20release/Nsfh3main04202005.CBK]
# -1, -2: missing
...
# RT1D1: assume more time spent for outdoor task as a male. Mean is 4.3, std.dev is 6.5, so threshold: 11
...
# Combine the 5 datasets into one tidy dataset and 
# save the dataset (preferably as an R data file).
...
# One graph should show individual heights (no summaries) 
# and be faceted by study.
...
# One other graph of your choice that tried to answer the question. 
# You could try creating a "decade" column and showing height summaries
# by decade.
